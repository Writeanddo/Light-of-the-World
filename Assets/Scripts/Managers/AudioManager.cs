...
public class AudioManager : Singleton<AudioManager>
{
    ...
    public void PlayMusic(SoundEffect musicClip)
    {
        // choose a random clip
        var clip = musicClip.clip;
        if (clip == null)
            clip = GetRandomClip(musicClip.clips);
    ...
    public SFXAudioSource Play(SoundEffect sfx)
    {
        var sfxSource = srcPool.Get();
        return sfxSource.Play(sfx);
    }

    public AudioSource Play(SoundEffect sfx, int clipIndex)
    {
        var sfxSource = srcPool.Get();
        sfxSource.Play(sfx, clipIndex);
